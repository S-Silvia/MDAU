...
def main():
    ...
    if opt.dataset == 'CUB':
        ...
        opt.resnet_path = '/media/cs4007/DATA/syy/try/pretrained_models/resnet101CUB_66.pth'
        ...
        if opt.awa_finetune:
            ...
            opt.resnet_path = '/media/cs4007/DATA/syy/try/pretrained_models/resnet101-5d3b4d8f.pth'
        ...
        sub_group_dic = {}
        opt.resnet_path = '/media/cs4007/DATA/syy/try/pretrained_models/resnet101_sun.pth.tar'
    else:
        ...
        for epoch in range(opt.nepoch):
            ...
            if (i + 1) == batch or (i + 1) % 200 == 0:   #batch=368
                ...
                if not opt.gzsl:
                    ...
                    if acc_GZSL_H > 72.1:
                        # save model state
                        model_save_path = os.path.join('/media/cs4007/DATA/syy/try/model/out/{}_GZSL_id_{}.pth'.format(opt.dataset, opt.train_id))
                        torch.save(model.state_dict(), model_save_path)
                        print('model saved to:', model_save_path)
...
f = open('Best_results.txt', 'a')  # 读取label.txt文件，没有则创建，‘a’表示再次写入时不覆盖之前的内容
    f.write('\n')
    f.write(str(result_gzsl.best_acc_U) + ' Best Unseen')
    f.write('\n')
    f.write(str(result_gzsl.best_acc_S) + ' Best Seen')
    f.write('\n')
    f.write(str(result_gzsl.best_acc) + ' Best HM')
    f.write('\n')
    f.write(str(opt.temp) + ' temp')
