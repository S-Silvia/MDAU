...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        #读取MATLAB文件，并将其转换为Python中的字典类型数据
        if opt.dataset == 'CUB':
            matcontent = sio.loadmat(opt.dataroot + "/" + "CUB_200_2011" + "/" + opt.image_embedding + ".mat")
        else:
            matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/" + opt.image_embedding + ".mat")
        # print("using the matcontent:", opt.dataroot + "/" + opt.dataset + "/" + opt.image_embedding + ".mat")
        feature = matcontent['features'].T   #(37322,2048) /图像特征 (11788, 2048)
        self.image_files = matcontent['image_files'].squeeze()  #图片路径
        self.label = matcontent['labels'].astype(int).squeeze() - 1  # (37322) /图像label  (11788,)
        if opt.dataset == 'CUB':
            matcontent = sio.loadmat(opt.dataroot + "/" + "CUB_200_2011" + "/" + opt.class_embedding + "_splits.mat")
        else:
        ...
        self.attribute = torch.from_numpy(matcontent['att'].T).float()  #(200,312) 类：200，属性：312
...
def prepare_attri_label(attribute, classes):
    # attribute.shape[50,85]:每一行表示该类对应85个属性的分数
    #classes.shape10/40/50:存储选中的类对应的标签（索引）0-49
    classes_dim = classes.size(0)    #10/40/50
    attri_dim = attribute.shape[1]  #85
    output_attribute = torch.FloatTensor(classes_dim, attri_dim) #[10,85]
    for i in range(classes_dim):
        output_attribute[i] = attribute[classes[i]]
    return torch.transpose(output_attribute, 1, 0)
#根据给定的类别和属性特征，从属性特征张量中选择与类别对应的属性，并返回一个转置后的属性特征张量
