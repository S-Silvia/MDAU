...
class SpatiaAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.conv1 = nn.Conv2d(2,312,kernel_size,padding=kernel_size//2,bias=False)
...
class Spectrum(nn.Module):
    def __init__(self,):
        ...
        self.SpatialAttention = SpatiaAttention()
        self.con1x1 = nn.Conv2d(in_channels=312, out_channels=312, kernel_size=1).to(device)
...
class resnet_proto_IoU(nn.Module):
    def __init__(self, opt,attribute_name):
        ...
        self.proto_net = nn.Sequential(
            nn.Linear(312,4096),    # attri_number
            nn.LeakyReLU(inplace=True),
            nn.Linear(4096,2048),
            nn.LeakyReLU(inplace=True),
        )
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv2d(312,1024,kernel_size=3,padding=1),    #102,85  #CUB:1024  SUN,AWA2:512
            nn.LeakyReLU(inplace=True),                     #CUB
            nn.AvgPool2d(kernel_size=2,stride=2),
            nn.Conv2d(1024,312,kernel_size=1),
            nn.LeakyReLU(inplace=True),
            nn.AdaptiveMaxPool2d(1),
        )

        emb_dim = attribute_name.shape[0]
    ...
    def forward(self,opt, x,attribute_class, return_map=False):
        ...
        #control_x = self.softmax(control_x).squeeze(-1).squeeze(-1)    #AWA2 SUN
